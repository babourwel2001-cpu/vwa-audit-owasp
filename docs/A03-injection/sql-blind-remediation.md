# SQL Injection Blind - Remédiation
...
Fichier : `/etc/apache2/conf-available/sqli-blind.conf`

Blocage des mots-clés SQL suivants : `SELECT`, `UNION`, `INSERT`, `DELETE`, `DROP`, `UPDATE`, `FROM`, `WHERE`, `OR`, `AND`, `--`.

**Restriction IP**
Accès au module SQL Injection Blind limité à une IP autorisée via `.htaccess`.
...
Résultat avant remédiation : `User ID exists in the database.`
